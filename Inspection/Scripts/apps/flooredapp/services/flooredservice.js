...
define(['app'], function (app) {
    ...
    var flooredFactory = function ($http, $q) {
        var serviceBase = '/api/Floored/',
            factory = {};

        factory.getFloored = function (pageIndex, pageSize) {
            return getPagedResource('Floored', pageIndex, pageSize);
        };

        factory.getFlooredSummary = function (pageIndex, pageSize) {
            return getPagedResource('FlooredSummary', pageIndex, pageSize);
        };
        ...
        factory.getCustomer = function (id) {
            ...
            return $http.get(serviceBase + 'customerById/' + id).then(function (results) {
                extendFloored([results.data]);
                return results.data;
            });
        };

        function extendFloored(Floored) {
            var custsLen = Floored.length;
            //Iterate through Floored
    ...
    FlooredFactory.$inject = injectParams;

    app.factory('FlooredService', FlooredFactory);
